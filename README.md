...
- `telegram-api` debe ser el nombre interno real del servicio Bot API local en Dokploy.
- Si tu servicio interno se llama distinto, cambia `telegram-api` por ese nombre.
...
- `MAX_CONCURRENT_UPDATES`: actualizaciones concurrentes del bot para multiusuario (default `16`).
- `MAX_PARALLEL_HEAVY_JOBS`: limite global de trabajos pesados en paralelo (compresion/extraccion/encriptado/renombrado).
- `ZIP_COMPRESS_LEVEL`: nivel de compresion ZIP (0-9). Para mas velocidad usa `1`.
- `CHECKSUM_MAX_BYTES`: tamano maximo para checksum SHA-256 (si el archivo supera este valor, se omite checksum para acelerar).
...
- `TELEGRAM_OPERATION_TIMEOUT`: timeout total por operacion de red (default recomendado `480` para archivos grandes).

### Perfil recomendado: Hostinger plan inicial (Dokploy)

Si tu VPS es de entrada, usa este perfil base para evitar cuelgues por saturacion:
...
- `CHECKSUM_MAX_BYTES=1073741824`
- `TELEGRAM_OPERATION_TIMEOUT=420`
- `ENABLE_CHECKSUM=1`

Si observas CPU por debajo de 60% y RAM estable, sube gradualmente:

- `MAX_CONCURRENT_UPDATES`: `8 -> 12 -> 16`
- `MAX_DOWNLOAD_CONCURRENCY`: `2 -> 3 -> 4`
- `MAX_UPLOAD_CONCURRENCY`: `2 -> 3`

### Perfil balanceado recomendado

Este es el punto de partida ideal para tantear entre estabilidad y velocidad:
...
Si buscas priorizar estabilidad, baja a:

- `MAX_DOWNLOAD_CONCURRENCY=2`
- `MAX_CONCURRENT_UPDATES=8`

Si buscas exprimir velocidad y la VPS aguanta, sube a:

- `MAX_DOWNLOAD_CONCURRENCY=3`
- `MAX_CONCURRENT_UPDATES=12`
- `CHECKSUM_MAX_BYTES=536870912`

### Perfiles listos para pegar
...
**Velocidad maxima razonable en 1 CPU**
...
ZIP_COMPRESS_LEVEL=1
CHECKSUM_MAX_BYTES=536870912
TELEGRAM_OPERATION_TIMEOUT=360
```

### Nota sobre Telegram Premium

Tener cuenta Premium en Telegram mejora limites para cuentas de usuario, pero no cambia automaticamente los limites de la Bot API del bot.
Para exprimir archivos grandes con bots, lo mas importante sigue siendo usar Bot API local y una configuracion de concurrencia estable en el servidor.
...
- `TELEGRAM_API_BASE_URL` (ejemplo: `http://telegram-api:8081/bot`)
- `TELEGRAM_API_FILE_BASE_URL` (ejemplo: `http://telegram-api:8081/file/bot`)
