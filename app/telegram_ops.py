...
async def retry_async(cfg: BotConfig, op_name: str, coro_factory: Callable[[], Awaitable[Any]]) -> Any:
    async def _sleep_with_cancel(delay: float, cancel_requested: Callable[[], bool] | None) -> None:
        if delay <= 0:
            return
    ...
    return await _retry_async_inner(cfg, op_name, coro_factory, cancel_requested=None, sleeper=_sleep_with_cancel)
...
async def _retry_async_inner(
    ...
    for attempt in range(1, cfg.max_retries + 1):
        if cancel_requested and cancel_requested():
            raise RuntimeError("Operacion cancelada por el usuario")
        try:
            return await asyncio.wait_for(coro_factory(), timeout=cfg.telegram_operation_timeout)
        except RetryAfter as retry_exc:
...
async def retry_async_with_cancel(
    ...
    return await _retry_async_inner(cfg, op_name, coro_factory, cancel_requested, _sleep_with_cancel)
...
async def upsert_progress(
    ...
    line1 = f"{stage_title} {spinner}"
    line2 = f"[{bar}]"
    line3 = f"Processing... : {display_percent:.2f}%"
    ...
    if uses_byte_progress:
        ...
        if bounded_current < prev_bytes or now <= prev_ts:
            prev_bytes = 0
            prev_ts = stage_started_at

        delta_bytes = bounded_current - prev_bytes
        delta_t = now - prev_ts
        ...
        if speed_bps <= 0 and bounded_current > 0 and elapsed > 0.5:
            speed_bps = int(bounded_current / elapsed)
        ...
        state["speed_last_bytes"] = bounded_current
        state["speed_last_ts"] = now

        if bounded_current >= total_safe:
            eta_text = "0s"
        elif speed_bps > 0:
            eta_seconds = (total_safe - bounded_current) / speed_bps
            eta_text = _format_duration(eta_seconds)

        line4 = f"{_format_iec_bytes(bounded_current)} of {_format_iec_bytes(total_safe)}"
        line5 = f"Speed : {_format_iec_bytes(max(0, speed_bps))}/s"
    else:
        line4 = "Data : calculating..."
        line5 = "Speed : 0 B/s"

    line6 = f"ETA : {eta_text}"
    last_value_change_ts = float(state.get("last_value_change_ts", now))
    idle_for = max(0.0, now - last_value_change_ts)
    activity_line = "Status : working"
    if not terminal_stage and idle_for > max(2.0, cfg.progress_min_update_seconds * 3):
        activity_line = "Status : still processing (no new bytes yet)"
    ...
    if not message_id:
        sent = await _retry_progress_op(lambda: context.bot.send_message(chat_id=chat.id, text=text, reply_markup=reply_markup))
        if sent:
            remember_message(update, context, sent.message_id)
            ...
            if "message is not modified" in err:
                ...
                try:
                    sent = await _retry_progress_op(
                        lambda: context.bot.send_message(chat_id=chat.id, text=text, reply_markup=reply_markup)
                    )
                    if sent:
                        remember_message(update, context, sent.message_id)
                        state["message_id"] = sent.message_id
...
async def send_path_with_size_control(
    ...
    if size <= effective_part_size(cfg):
        async def _single_upload():
            return await send_file_with_best_method(
                ...
                timeout_seconds=cfg.telegram_operation_timeout,
            )

        message_id = await retry_async_with_cancel(cfg, f"Subida {file_path.name}", _single_upload, cancel_requested)
        return [message_id]

    ids: list[int] = []
    if cancel_requested and cancel_requested():
        raise RuntimeError("Operacion cancelada por el usuario")
    parts = await asyncio.to_thread(split_file_for_upload, file_path, cfg, cancel_requested)
        ...
        async def _upload_part():
            with part.open("rb") as payload:
                sent = await asyncio.wait_for(
                    ...
                    timeout=cfg.telegram_operation_timeout,
                )
            return sent.message_id

        sent_id = await retry_async_with_cancel(cfg, f"Subida parte {part.name}", _upload_part, cancel_requested)
        ids.append(sent_id)
    return ids
