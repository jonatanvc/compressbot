    ...
    for attempt in range(1, cfg.max_retries + 1):
        ...
        try:
            ...
            while True:
                ...
                if task in done:
                    return await task
                if (now_monotonic() - start_ts) >= cfg.telegram_operation_timeout:
                    task.cancel()
                    try:
                        await task
                    except Exception:
...
async def send_path_with_size_control(
    ...
    upload_timeout = timeout_seconds if timeout_seconds is not None else cfg.telegram_operation_timeout
    if size <= part_size:
