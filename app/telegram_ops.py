...
async def send_path_with_size_control(
    ...
    upload_timeout = min(upload_timeout, max(60.0, min(base_upload_timeout, 180.0)))
    if size <= part_size:
    ...
    def _should_split_smaller(exc: Exception) -> bool:
        if isinstance(exc, (asyncio.TimeoutError,)):  # noqa: PERF203 - explicit tuple for clarity
            return True
        text = str(exc).lower()
    ...
    for size_index, candidate_size in enumerate(fallback_sizes):
        ...
        try:
            for index, part in enumerate(parts, start=1):
                ...
                part_timeout = min(part_timeout, max(600.0, min(base_upload_timeout, 1800.0)))
                caption = (
