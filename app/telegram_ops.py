...
async def upsert_progress(
    ...
    if "descarg" in stage_lower:
        ...
        stage_title = "ENCRIPTANDO ARCHIVO"
        stage_emoji = "🔐"
    elif "renombr" in stage_lower:
        stage_title = "RENOMBRANDO ARCHIVO"
        stage_emoji = "✏️"
    else:
    ...
    bar = progress_bar(display_percent, cfg.progress_bar_size, filled_char="✦", empty_char="◌")
    spinner = _progress_spinner(elapsed)
    line1 = f"{spinner} {stage_emoji} {stage_title}"
    line2 = f"{bar} {display_percent:.0f}%"
            ...
            with file_path.open("rb") as payload:
                if ext in IMAGE_EXTENSIONS:
                    sent_message = await asyncio.wait_for(
                        context.bot.send_photo(chat_id=chat_id, photo=payload, reply_markup=reply_markup),
                        timeout=timeout_seconds,
                    )
                elif ext in AUDIO_EXTENSIONS:
                    sent_message = await asyncio.wait_for(
                        context.bot.send_audio(chat_id=chat_id, audio=payload, reply_markup=reply_markup),
                        timeout=timeout_seconds,
                    )
                elif ext in VIDEO_EXTENSIONS:
                    sent_message = await asyncio.wait_for(
                        context.bot.send_video(chat_id=chat_id, video=payload, reply_markup=reply_markup),
                        timeout=timeout_seconds,
                    )
                else:
                    sent_message = await asyncio.wait_for(
