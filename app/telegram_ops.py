...
def _probe_media(file_path: Path) -> dict[str, Any]:
    """Run ffprobe on *file_path* and return a dict with 'duration' (float|None)
    and 'width'/'height' (int|None) for video streams."""
    try:
...
def _extract_video_thumbnail(file_path: Path, out_dir: Path) -> Path | None:
    """Extract a thumbnail image at mid-point of the video using ffmpeg.
    Returns the thumbnail path or None if it fails."""
    try:
        import subprocess as _sp
        thumb_path = out_dir / f"__thumb_{file_path.stem}.jpg"
        # Seek to ~3s or beginning; -vframes 1 extracts one frame.
        result = _sp.run(
            [
                "ffmpeg", "-y", "-ss", "3", "-i", str(file_path),
                "-vframes", "1", "-q:v", "5",
                "-vf", "scale=320:-1",
                str(thumb_path),
            ],
