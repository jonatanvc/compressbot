        ...
        try:
            with file_path.open("rb") as payload:
                if ext in IMAGE_EXTENSIONS:
                    ...
                    # Enviar audio como documento para preservar el archivo
                    # original bit-a-bit. send_audio() causa que Telegram
                    # re-codifique el archivo (a OGG/OPUS), lo cual puede
                    # producir silencio o artefactos en muestras cortas,
                    # formatos poco comunes o tasas de muestreo inusuales.
                    sent_message = await asyncio.wait_for(
...
async def send_path_with_size_control(
    ...
    all_fallback_sizes = [
        part_size,
        min(512 * 1024 * 1024, part_size),
        min(256 * 1024 * 1024, part_size),
        128 * 1024 * 1024,
    ]
