...
from .constants import IMAGE_EXTENSIONS, KEY_PROGRESS, MAX_DIRECT_UPLOAD_BYTES, VIDEO_EXTENSIONS
...
class _ProgressReader:
    """Wraps a binary file object, reporting read progress to a state dict."""
    ...
    # httpx's multipart encoder calls read(CHUNK_SIZE) repeatedly.
    def read(self, size: int = -1) -> bytes:
        ...
        if self._state is not None:
            if chunk:
                ...
                self._state["details"] = "Esperando confirmación de Telegram..."
        return chunk
    ...
    for _ in range(3):
        try:
            if ext in IMAGE_EXTENSIONS:
                ...
                # Documentos (y audio): streaming upload para no cargar
