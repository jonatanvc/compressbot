...
async def upsert_progress(
    ...
    if uses_byte_progress:
        ...
        if speed_bps > 0 and bounded_current < total_safe:
        ...
        line5 = f"Speed : {_format_iec_bytes(speed_bps)}/s" if speed_bps > 0 else "Speed : calculating..."
    else:
        line4 = "Data : calculating..."
        line5 = "Speed : calculating..."
    ...
    if not message_id:
        ...
        try:
            ...
            if "message is not modified" in err:
                ...
                # Si Telegram ya no permite editar este mensaje (borrado/no editable), recrea progreso y continua.
                sent = await context.bot.send_message(chat_id=chat.id, text=text, reply_markup=reply_markup)
                remember_message(update, context, sent.message_id)
                state["message_id"] = sent.message_id
            else:
