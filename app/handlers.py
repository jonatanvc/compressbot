...
class BotHandlers:
    ...
    async def _extract_nested_archives(
        ...
        extracted_count = 0
        failed_archives: set[str] = set()
        processed: set[str] = set()
        pending = [path for path in collect_extracted_files(base_dir) if is_archive_file(path.name)]
        max_nested = 40

        while pending and len(processed) < max_nested:
            if bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                break
            ...
            nested_out = base_dir / f"{archive_path.stem}_{uuid.uuid4().hex[:8]}"
            ...
            try:
                ...
                extracted_count += 1
            except (PasswordRequiredError, InvalidPasswordError, MissingRarBackendError, Missing7zBackendError) as exc:
                failed_archives.add(resolved)
                logger.warning("Nested extraction skipped for %s: %s", archive_path, exc)
                continue
            except Exception as exc:
                failed_archives.add(resolved)
                logger.warning("Nested extraction failed for %s: %s", archive_path, exc)
                continue

            for child in collect_extracted_files(nested_out):
                if is_archive_file(child.name):
        ...
        if pending:
            for leftover in pending:
                if leftover.exists():
                    try:
                        failed_archives.add(str(leftover.resolve()))
                    except Exception:
                        failed_archives.add(str(leftover))

        return extracted_count, failed_archives
    ...
    async def _extract_local_targets(
        ...
        for index, local_archive in enumerate(targets, start=1):
            ...
            nested_failed: set[str] = set()
            try:
                _, nested_failed = await self._extract_nested_archives(context, out_dir, current_password)
            except Exception as exc:
                logger.warning("Nested extraction scan failed for %s: %s", out_dir, exc)

            extracted_payloads = list(collect_extracted_files(out_dir))
            if extracted_payloads:
                if nested_failed:
                    def _is_failed_archive(path: Path) -> bool:
                        try:
                            return str(path.resolve()) in nested_failed
                        except Exception:
                            return str(path) in nested_failed

                    extracted_payloads = [
                        path
                        for path in extracted_payloads
                        if (not is_archive_file(path.name))
                        or _is_failed_archive(path)
                    ]
                else:
                    extracted_payloads = [path for path in extracted_payloads if not is_archive_file(path.name)]
    ...
    async def _handle_password_input_locked(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if "cancelada" in str(exc).lower():
                ...
                context.user_data[KEY_ARCHIVES_TO_EXTRACT] = []
                context.user_data[KEY_PENDING_LOCAL_TARGETS] = []
                await self._cleanup_progress_message(update, context, delete_message=False, preserve_message_id=True)
                await tracked_reply(update, context, "Extraccion cancelada por el usuario.", reply_markup=start_keyboard())
                return
            context.user_data[KEY_MODE] = MODE_WAIT_EXTRACT_ARCHIVES
            await self._cleanup_progress_message(update, context, delete_message=False, preserve_message_id=True)
