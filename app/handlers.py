...
from .runtime import get_user_lock
from .telegram_ops import format_bot_text, retry_async, send_path_with_size_control, tracked_reply, upsert_progress
...
class BotHandlers:
    ...
    async def _download_file_robust(
        ...
        try:
            await retry_async(self.cfg, label, _download_to_drive_once)
            ...
            await retry_async(self.cfg, f"{label} (fallback SSL)", _download_as_bytes_once)
    ...
    async def _extract_local_targets(
        ...
        for index, local_archive in enumerate(targets, start=1):
            ...
            try:
                ...
                await run_extract_in_thread(local_archive, out_dir, password=current_password)
    ...
    async def finalize_compression(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        effective_download_concurrency = self.cfg.max_download_concurrency
        if len(files_to_compress) >= 15:
            effective_download_concurrency = min(effective_download_concurrency, 2)
        download_sem = asyncio.Semaphore(max(1, effective_download_concurrency))
        ...
        async with user_lock:
            try:
                ...
                async def download_one(item: dict[str, Any]) -> None:
                    ...
                    async with download_sem:
                        ...
                        try:
                            ...
                            if self.cfg.enable_checksum:
                                _ = await compute_sha256_async(target)
                            ...
                            async with progress_lock:
                                failed_downloads.append(str(item.get("file_name", "archivo")))
                ...
                if failed_downloads:
                    preview = ", ".join(failed_downloads[:5])
                    extra = "" if len(failed_downloads) <= 5 else f" y {len(failed_downloads) - 5} mas"
                    await tracked_reply(
                        update,
                        context,
                        (
                            f"Aviso operativo: {len(failed_downloads)} archivo(s) no se pudieron descargar y fueron omitidos: "
                            f"{preview}{extra}"
                        ),
                    )
                ...
                if self.cfg.enable_checksum:
                    _ = await compute_sha256_async(archive_path)

                output_parts = await asyncio.to_thread(split_file_for_upload, archive_path, self.cfg)
    ...
    async def process_extract_queue(self, update: Update, context: ContextTypes.DEFAULT_TYPE, password: str | None = None) -> bool:
        ...
        await upsert_progress(
            ...
            0,
            max(initial_queue_total, 1),
            f"0/{initial_queue_total} archivos descargados | 0 B recibidos",
            force=True,
            cancellable=True,
        )
        ...
        while queue:
            ...
            try:
                ...
                await tracked_reply(
                    update,
                    context,
                    f"No se pudo descargar '{item['file_name']}'. Se omitira este archivo. Detalle: {exc}",
                    reply_markup=extract_keyboard(),
                )
                continue
            validate_file_ready(local_archive)
            if self.cfg.enable_checksum:
                _ = await compute_sha256_async(local_archive)
        ...
        async def upload_extracted_path(file_path: Path) -> None:
            ...
            async with upload_sem:
                ...
                async with progress_lock:
                    ...
                    await upsert_progress(update, context, self.cfg, "Subiendo archivos extraidos", sent_parts, max(total_upload_parts, 1), f"{sent_parts}/{max(total_upload_parts, 1)} partes subidas", cancellable=True)
    ...
    async def handle_password_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            await run_extract_in_thread(archive_path, output_dir, password=password)
        except InvalidPasswordError:
            context.user_data[KEY_MODE] = MODE_WAIT_PASSWORD
    ...
    def _encrypt_zip_as_7z(self, source_zip: Path, encrypted_7z: Path, password: str) -> None:
        ...
        try:
            ...
            proc = subprocess.run(cmd, cwd=str(extracted_dir), capture_output=True, text=True)
    ...
    async def handle_encrypt_password_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            await upsert_progress(update, context, self.cfg, "Encryptando archivo", 0, 1, "Preparando archivo", force=True)
            ...
            await asyncio.to_thread(self._encrypt_zip_as_7z, source_path, temp_encrypted, password)
            ...
            await upsert_progress(update, context, self.cfg, "Encryptando archivo", 1, 1, "Subiendo archivo encryptado", force=True)
    ...
    async def handle_rename_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            await upsert_progress(update, context, self.cfg, "Renombrando archivo", 0, 1, "Preparando archivo", force=True)
            await self._download_pending_file(context, source_path)
            ...
            await upsert_progress(update, context, self.cfg, "Renombrando archivo", 1, 1, "Subiendo archivo renombrado", force=True)
