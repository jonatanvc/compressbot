...
class BotHandlers:
    ...
    async def _extract_local_targets(
        ...
        for index, local_archive in enumerate(targets, start=1):
            ...
            # --- Bot split (.partNNN) → rejoin before extraction ---------------
            joined_tmp: Path | None = None
            if is_bot_split_part(local_archive.name):
                base_name = bot_split_base_name(local_archive.name).lower()
                ...
                for sibling in parent_dir.iterdir():
                    ...
                    sib_base = bot_split_base_name(sib_lower)
                    if sib_base != base_name:
                        continue
                    m = re.search(r"\.part(\d{3})$", sib_lower)
                    if m:
                        parts.append((int(m.group(1)), sibling))
                parts.sort(key=lambda t: t[0])
                if parts:
                    joined_name = bot_split_base_name(local_archive.name)
                    joined_tmp = out_dir / f"__joined_{joined_name}"
                    self._raise_if_cancel_requested(context)
                    await upsert_progress(
                        update, context, self.cfg,
                        "Reuniendo partes",
                        0, max(sum(p.stat().st_size for _, p in parts), 1),
                        f"Uniendo {len(parts)} partes → {joined_name}",
    ...
    async def queue_file_for_compression(self, update: Update, context: ContextTypes.DEFAULT_TYPE, file_info: dict[str, Any]) -> None:
        ...
        archive_warning = ""
        if is_archive_file(file_info["file_name"]):
            archive_warning = (
                "\n⚠️ Este archivo parece un comprimido. "
                "Si deseas extraerlo, cancela y usa Extraer."
            )
        ...
        queue_text = (
            ...
            f"Pulsa Finalizar compresion para iniciar el proceso.{archive_warning}"
        )
