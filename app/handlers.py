...
class BotHandlers:
    ...
    async def _extract_local_targets(
        ...
        for index, local_archive in enumerate(targets, start=1):
            ...
            try:
                await upsert_progress(
                    ...
                    (
                        ...
                        f"{format_bytes(min(extract_bytes_done, total_extract_bytes))} de {format_bytes(total_extract_bytes)}"
                    ),
                    cancellable=True,
                )
            ...
            extracted_payloads = [
                path for path in collect_extracted_files(out_dir)
        ...
        async with user_lock:
            ...
            try:
                ...
                if should_fallback_to_split:
                    fallback_sizes = [2 * 1024 * 1024 * 1024, 1024 * 1024 * 1024, 768 * 1024 * 1024, 512 * 1024 * 1024]
                    fallback_index = 0
                    while True:
                        _cleanup_parts_dir()
    ...
    async def process_extract_queue(self, update: Update, context: ContextTypes.DEFAULT_TYPE, password: str | None = None) -> bool:
        ...
        try:
            ...
            await upsert_progress(
                ...
                f"0/{max(total_upload_parts, 1)} partes | {format_bytes(0)} de {format_bytes(total_upload_bytes)}",
                force=True,
                cancellable=True,
                percent_override=0.0,
            )
                ...
                async with upload_sem:
                    ...
                    if remaining_parts_after_this == 0 and sent_parts < total_upload_parts:
                        ...
                        await upsert_progress(
                            ...
                            max(total_upload_bytes, 1),
                            f"Subiendo último archivo: {file_path.name}",
                            cancellable=True,
                            percent_override=min(99.0, (sent_parts / max(total_upload_parts, 1)) * 100.0 + 4.0),
                        )
                    ...
                    async def _on_part_sent(part_index: int, total_parts: int, message_id: int) -> None:
                        ...
                        async with progress_lock:
                            ...
                            await upsert_progress(
                                ...
                                cancellable=True,
                                percent_override=percent_parts,
                            )
                    ...
                    async with progress_lock:
                        ...
                        await upsert_progress(
                            ...
                            (
                                f"{sent_parts}/{max(total_upload_parts, 1)} partes subidas | "
                                f"{format_bytes(min(uploaded_bytes, total_upload_bytes))} de {format_bytes(total_upload_bytes)}"
                            ),
                            cancellable=True,
                            percent_override=(sent_parts / max(total_upload_parts, 1)) * 100.0,
                        )

            upload_queue: asyncio.Queue[Path] = asyncio.Queue()
            for file_path in paths_to_send:
                upload_queue.put_nowait(file_path)
            ...
            async def _upload_worker() -> None:
                while True:
                    try:
                        queued_path = upload_queue.get_nowait()
                    except asyncio.QueueEmpty:
                        return
                    try:
        ...
        async with user_lock:
            try:
                ...
                if "cancelada" in str(exc).lower():
                    ...
                    await self._cleanup_progress_message(update, context, force=True, delete_message=False)
    ...
    async def _handle_password_input_locked(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if "cancelada" in str(exc).lower():
                ...
                context.user_data[KEY_ARCHIVES_TO_EXTRACT] = []
                context.user_data[KEY_PENDING_LOCAL_TARGETS] = []
                await self._cleanup_progress_message(update, context, delete_message=False)
                await tracked_reply(update, context, "Extraccion cancelada por el usuario.", reply_markup=start_keyboard())
                return
            context.user_data[KEY_MODE] = MODE_WAIT_EXTRACT_ARCHIVES
            await self._cleanup_progress_message(update, context, delete_message=False)
    ...
    async def _download_pending_file_multipart_if_needed(
        ...
        if not parts_meta:
            target_path = unique_target_path(target_dir, preferred_name)
            await self._download_pending_file(context, target_path)
            return target_path
