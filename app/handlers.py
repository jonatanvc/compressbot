...
from .runtime import get_global_heavy_job_semaphore, get_user_lock
...
class BotHandlers:
    ...
    async def _handle_encrypt_password_input_locked(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await heavy_gate.acquire()
        try:
            await upsert_progress(update, context, self.cfg, "Encryptando archivo", 0, 4, "Preparando descarga", force=True, cancellable=True)
            heartbeat_task = self._start_progress_heartbeat(update, context)
            await self._download_pending_file(context, source_path)
            self._raise_if_cancel_requested(context)
            await upsert_progress(update, context, self.cfg, "Encryptando archivo", 1, 4, "Archivo descargado. Preparando cifrado 7z", force=True, cancellable=True)
            ...
            self._raise_if_cancel_requested(context)
            await upsert_progress(update, context, self.cfg, "Encryptando archivo", 2, 4, "Cifrado completado. Validando archivo", force=True, cancellable=True)
            ...
            await upsert_progress(update, context, self.cfg, "Encryptando archivo", 3, 4, "Archivo validado. Subiendo archivo encryptado", force=True, cancellable=True)
            ...
            await upsert_progress(update, context, self.cfg, "Encryptando archivo", 4, 4, "Proceso finalizado correctamente", force=True)
        except RuntimeError as exc:
            if "cancelada" in str(exc).lower():
                await upsert_progress(update, context, self.cfg, "Cancelando", 1, 1, "La operación fue cancelada por el usuario.", force=True)
    ...
    async def _handle_rename_input_locked(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await heavy_gate.acquire()
        try:
            await upsert_progress(update, context, self.cfg, "Renombrando archivo", 0, 3, "Preparando descarga", force=True, cancellable=True)
            heartbeat_task = self._start_progress_heartbeat(update, context)
            await self._download_pending_file(context, source_path)
            self._raise_if_cancel_requested(context)
            await upsert_progress(update, context, self.cfg, "Renombrando archivo", 1, 3, "Archivo descargado. Aplicando nuevo nombre", force=True, cancellable=True)

            renamed_path = source_path.with_name(requested_name)
            source_path.rename(renamed_path)
            validate_file_ready(renamed_path)

            await upsert_progress(update, context, self.cfg, "Renombrando archivo", 2, 3, "Nombre aplicado. Subiendo archivo renombrado", force=True, cancellable=True)

            await self._send_result_with_actions(update, context, renamed_path, "Archivo renombrado correctamente.")
            await upsert_progress(update, context, self.cfg, "Renombrando archivo", 3, 3, "Proceso finalizado correctamente", force=True)
        except RuntimeError as exc:
            if "cancelada" in str(exc).lower():
                await upsert_progress(update, context, self.cfg, "Cancelando", 1, 1, "La operación fue cancelada por el usuario.", force=True)
