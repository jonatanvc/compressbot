...
from .constants import (
    ...
    KEY_COMPRESS_NAME,
    KEY_COMPRESS_PASSWORD,
    KEY_COMPRESS_NAME_MSG_ID,
    KEY_COMPRESS_PWD_PROMPT_MSG_ID,
    ...
    MODE_WAIT_COMPRESS_NAME,
    MODE_WAIT_COMPRESS_PASSWORD,
...
from .keyboards import compressed_result_keyboard, compress_keyboard, compress_password_keyboard, extract_keyboard, favorite_channel_keyboard, favorite_keyboard, start_keyboard, waiting_upload_keyboard
from .runtime import get_global_heavy_job_semaphore, get_user_lock, get_user_queue_ui_lock, register_active_task, unregister_active_task, cancel_active_task
from .telegram_ops import format_bot_text, retry_async_with_cancel, send_path_with_size_control, stream_send_document, tracked_reply, upsert_progress
from .web_downloads import (
    download_google_drive,
    extract_google_drive_url,
    fetch_gdrive_file_size_async,
...
class BotHandlers:
    ...
    async def _download_file_robust(
        ...
        # Lambda de cancelación disponible para todos los helpers anidados
        def _is_cancelled() -> bool:
            return bool(context.user_data.get(KEY_CANCEL_REQUESTED, False))
        ...
        async def _download_to_drive_once() -> None:
            ...
            meta_timeout = max(300.0, self.cfg.telegram_connect_timeout + self.cfg.telegram_pool_timeout + 120.0)
            ...
            async def _with_polling() -> None:
                ...
                try:
                    while not task.done():
                        # Cancelación inmediata: si el flag está activo, abortar la descarga.
                        # El task.cancel() propaga la cancelación instantáneamente; el polling
                        # solo necesita un check ligero, sin llamadas a stat().
                        if _is_cancelled():
                            await _cancel_task_gracefully(grace_seconds=1.0)
        ...
        try:
            await retry_async_with_cancel(
                ...
                cancel_requested=_is_cancelled,
    ...
    async def _cleanup_progress_message(
        ...
        if delete_message and progress_ids:
            # Desanclar antes de borrar
            for pid in dict.fromkeys(progress_ids):
                try:
                    await context.bot.unpin_chat_message(chat_id=chat.id, message_id=int(pid))
                except Exception:
                    pass
            await asyncio.gather(
    ...
    def _recommended_upload_timeout(size_bytes: int, base_timeout: float) -> float:
        ...
        # NO usar base_timeout (operation_timeout global) como piso para partes
    ...
    def _split_extract_targets(self, downloaded_files: list[Path]) -> tuple[list[Path], list[str]]:
        ...
        for path in files_sorted:
            ...
            # Bot's own split format: file.zip.part001, file.zip.part002, ...
            if is_bot_split_part(lower_name):
                match = re.search(r"\.part(\d{3})$", lower_name)
                if not match:
                    continue
                ...
                if part_number == 1:
                    ...
                    first_part = re.sub(r"\.part\d{3}$", ".part001", lower_name)
            ...
            if re.search(r"\.7z\.\d{3}$", lower_name):
                # Puede ser .7z.001 puro o compuesto (.rar.7z.001)
                # Ambos se manejan con la primera parte como target
            ...
            # Plain numeric split: file.001, file.002, ... (7-Zip / HJSplit)
            plain_num_match = re.search(r"\.(\d{3})$", lower_name)
            if plain_num_match:
                part_number = int(plain_num_match.group(1))
                if part_number == 1:
                    targets.append(path)
                else:
                    first_part = re.sub(r"\.\d{3}$", ".001", lower_name)
                    if first_part not in name_map:
                        warnings.append(f"Falta {first_part} para iniciar serie de {path.name}.")
                continue
    ...
    async def _extract_local_targets(
        ...
        for index, local_archive in enumerate(targets, start=1):
            ...
            # --- Reunir partes split antes de extraer --------------------------
            joined_tmp: Path | None = None
            needs_rejoin = False
            rejoin_parts: list[tuple[int, Path]] = []
            rejoin_target_name: str = local_archive.name

            lower_archive = local_archive.name.lower()

            if is_bot_split_part(lower_archive):
                # Bot's own format: file.zip.part001, file.zip.part002
                needs_rejoin = True
                base_name = bot_split_base_name(lower_archive)
                rejoin_target_name = bot_split_base_name(local_archive.name)
                parent_dir = local_archive.parent
                for sibling in parent_dir.iterdir():
                    if not sibling.is_file():
                        continue
                    sib_base = bot_split_base_name(sibling.name.lower())
                    if sib_base != base_name:
                        continue
                    m = re.search(r"\.part(\d{3})$", sibling.name.lower())
                    if m:
                        rejoin_parts.append((int(m.group(1)), sibling))

            elif re.search(r"\.\d{3}$", lower_archive) and not re.search(r"\.(7z|zip|rar)\.\d{3}$", lower_archive):
                # Plain numeric split: file.001, file.002 (7-Zip split / HJSplit)
                needs_rejoin = True
                base_name = re.sub(r"\.\d{3}$", "", lower_archive)
                rejoin_target_name = re.sub(r"\.\d{3}$", "", local_archive.name)
                parent_dir = local_archive.parent
                for sibling in parent_dir.iterdir():
                    if not sibling.is_file():
                        continue
                    sib_base = re.sub(r"\.\d{3}$", "", sibling.name.lower())
                    if sib_base != base_name:
                        continue
                    m = re.search(r"\.(\d{3})$", sibling.name.lower())
                    if m:
                        rejoin_parts.append((int(m.group(1)), sibling))

            elif re.search(r"\.(7z|zip|rar)\.\d{3}$", lower_archive):
                # Multipart nativo de 7z/zip/rar: .7z.001, .zip.001, .rar.001
                # Pero si hay extensiones compuestas como .rar.7z.001, necesitamos
                # rejoin manual porque 7z puede confundirse con el nombre.
                # Deteccion: si hay MAS de una extension antes del .NNN, rejoin manual.
                stem_no_num = re.sub(r"\.\d{3}$", "", lower_archive)
                # Contar cuantas extensiones tipo archivo tiene el stem
                ext_count = len(re.findall(r"\.(7z|zip|rar|tar|gz|bz2|xz)", stem_no_num))
                if ext_count <= 1:
                    # Nativo: .7z.001 puro, .zip.001 puro, .rar.001 puro
                    # 7z/extract_archive maneja nativamente sin rejoin manual
                    pass
                else:
                    # Extension compuesta: .rar.7z.001, etc. — rejoin manual
                    needs_rejoin = True
                    base_name = re.sub(r"\.\d{3}$", "", lower_archive)
                    rejoin_target_name = re.sub(r"\.\d{3}$", "", local_archive.name)
                    parent_dir = local_archive.parent
            ...
            if needs_rejoin and rejoin_parts:
                rejoin_parts.sort(key=lambda t: t[0])
                joined_tmp = out_dir / f"__joined_{rejoin_target_name}"
                self._raise_if_cancel_requested(context)
                total_rejoin = max(sum(p.stat().st_size for _, p in rejoin_parts), 1)
                await upsert_progress(
                    update, context, self.cfg,
                    "Reuniendo partes",
                    0, total_rejoin,
                    f"Uniendo {len(rejoin_parts)} partes → {rejoin_target_name}",
                    force=True, cancellable=True,
                )

                def _do_join() -> None:
                    with joined_tmp.open("wb") as out:
                        for _, part_path in rejoin_parts:
                            if bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                                raise RuntimeError("Operacion cancelada por el usuario")
            ...
            try:
            ...
            except (PasswordRequiredError, InvalidPasswordError) as exc:
                # Para partes divididas con contraseña: si se creó un joined_tmp, éste sigue
                # existiendo (no se borró porque la extracción no terminó bien). Lo usamos
                # como archive_path para el reintento; el out_dir vacío también se conserva.
                archive_for_retry = local_archive  # puede ser joined_tmp o el archivo original
                is_invalid = isinstance(exc, InvalidPasswordError)
                prompt_msg = (
                    "Password incorrecta. Escribe la contraseña correcta para reintentar."
                    if is_invalid
                    else f"El archivo '{archive_for_retry.name}' está protegido con contraseña. Escríbela para continuar."
                )
                await self._cleanup_password_prompt_message(update, context)
                context.user_data[KEY_CURRENT_ARCHIVE] = {
                    "file_name": archive_for_retry.name,
                    "local_archive": str(archive_for_retry),
                    "output_dir": str(out_dir),
                }
                # Incluir el target actual (index-1 en base-1 = el que falló) para reintentarlo,
                # más los siguientes. Usamos targets[index-1:] ya que el bucle es enumerate(..., start=1)
                # y el fallido es el índice actual (index); targets[index-1] == local_archive original.
                remaining_original_targets = targets[index - 1:] if joined_tmp is None else targets[index:]
                context.user_data[KEY_PENDING_LOCAL_TARGETS] = [str(path) for path in remaining_original_targets]
                ...
                await upsert_progress(
                    ...
                    f"{archive_for_retry.name}",
                    force=True,
                    cancellable=True,
                )
                await self._upsert_password_prompt(update, context, prompt_msg)
                return False
                ...
                # Detectar errores de contraseña que vienen como RuntimeError desde 7z/rar.
                exc_text = str(exc).lower()
                _pwd_markers = ("wrong password", "bad password", "password", "bad decrypt",
                                "failed the read enough data", "mac check failed", "can not open encrypted")
                if any(marker in exc_text for marker in _pwd_markers) and "cancelada" not in exc_text:
                    archive_for_retry = local_archive
                    await self._cleanup_password_prompt_message(update, context)
                    context.user_data[KEY_CURRENT_ARCHIVE] = {
                        "file_name": archive_for_retry.name,
                        "local_archive": str(archive_for_retry),
                    ...
                    await self._upsert_password_prompt(
                        update, context,
                        f"El archivo '{archive_for_retry.name}' requiere contraseña o la actual es incorrecta. Escríbela para continuar.",
                    )
                    return False
                # Si falló la extracción pero tenemos un archivo reunido (joined):
                # - Si es un archivo comprimido reconocido, pedir contraseña
                #   (evita "reenviar" el archivo como si fuera el resultado).
                # - Si NO es archivo comprimido, tratarlo como resultado
                #   (el usuario simplemente quería juntar partes).
                if joined_tmp is not None and joined_tmp.exists() and joined_tmp.stat().st_size > 0:
                    if is_archive_file(joined_tmp.name):
                        logger.info(
                            "Extraction failed for joined archive %s (%s); prompting for password.",
                            joined_tmp.name, exc,
                        )
                        archive_for_retry = joined_tmp
                        await self._cleanup_password_prompt_message(update, context)
                        context.user_data[KEY_CURRENT_ARCHIVE] = {
                            "file_name": archive_for_retry.name,
                            "local_archive": str(archive_for_retry),
                        ...
                        remaining_original_targets = targets[index:]
                        context.user_data[KEY_PENDING_LOCAL_TARGETS] = [str(path) for path in remaining_original_targets]
                        context.user_data[KEY_MODE] = MODE_WAIT_PASSWORD
                        await upsert_progress(
                            update, context, self.cfg,
                        ...
                        await self._upsert_password_prompt(
                            update, context,
                            f"El archivo '{rejoin_target_name}' requiere contraseña o no se pudo extraer. Escríbela para continuar.",
                        )
                        return False
                    logger.info("Extraction failed for joined file %s (%s); treating joined file as result.", joined_tmp.name, exc)
            ...
            if joined_tmp is not None:
                if extraction_failed:
                    # Renombrar el archivo reunido para quitar el prefijo __joined_
                    final_joined = joined_tmp.parent / rejoin_target_name
                    try:
                        joined_tmp.rename(final_joined)
                    except Exception:
                        final_joined = joined_tmp
                    # Mantener el archivo reunido como resultado para el usuario
                    extracted_payloads = [p for p in extracted_payloads if p != joined_tmp and p != final_joined] + [final_joined]
                else:
                    # Extracción exitosa: eliminar el temp y excluirlo
                    try:
                        joined_tmp.unlink(missing_ok=True)
                    except Exception:
                        pass
                    extracted_payloads = [p for p in extracted_payloads if p != joined_tmp]
    ...
    async def _zip_path_with_progress(
        self,
        update: Update,
        context: ContextTypes.DEFAULT_TYPE,
        source_path: Path,
        ...
        if not source_path.exists():
            raise FileNotFoundError(f"No existe la ruta a comprimir: {source_path.name}")

        if source_path.is_file():
            files_for_zip = [source_path]
        ...
        total_bytes = max(1, sum(int(path.stat().st_size) for path in files_for_zip))
        await upsert_progress(
            update,
            context,
            self.cfg,
            ...
            0,
            total_bytes,
            f"{detail_prefix}: 0/{len(files_for_zip)}",
            force=True,
            cancellable=True,
        )
        ...
        def _build_zip() -> None:
            ...
            with zipfile.ZipFile(
                ...
                for index, file_path in enumerate(files_for_zip, start=1):
                    ...
                    arcname = file_path.relative_to(relative_base).as_posix()
                    with file_path.open("rb") as src, zf.open(arcname, "w", force_zip64=True) as dst:
                        while True:
                            if bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                                raise RuntimeError("Operacion cancelada por el usuario")
        ...
        while not build_task.done():
            ...
            if (now_ts - last_report_ts) >= 1.0:
                ...
                await upsert_progress(
                    ...
                    total_bytes,
                    (
                        f"{detail_prefix}: {min(file_index, len(files_for_zip))}/{len(files_for_zip)}"
                        f" | {format_bytes(min(bytes_done, total_bytes))} de {format_bytes(total_bytes)}"
                        f" | {file_name}"
                    ),
                    cancellable=True,
                )
        ...
        await build_task
        validate_file_ready(zip_target)
        await upsert_progress(
            update,
            context,
            self.cfg,
            ...
            total_bytes,
            total_bytes,
            f"{detail_prefix}: {len(files_for_zip)}/{len(files_for_zip)}",
            force=True,
            cancellable=True,
        )
        return zip_target

    async def _upload_path_for_drive_flow(
        self,
        update: Update,
        context: ContextTypes.DEFAULT_TYPE,
        path_to_send: Path,
        ...
        upload_size = max(1, int(path_to_send.stat().st_size))
        await upsert_progress(
            update,
            context,
            self.cfg,
            ...
            0,
            upload_size,
            f"Preparando envio de {path_to_send.name}",
            force=True,
            cancellable=True,
        ...
        sent_parts = 0
        expected_parts = max(1, math.ceil(upload_size / max(1, int(self.cfg.single_upload_max_bytes))))

        async def _on_part_sent(part_index: int, total_parts: int, _message_id: int) -> None:
            nonlocal sent_parts
            sent_parts = max(sent_parts, int(part_index))
            percent_parts = (sent_parts / max(total_parts, 1)) * 100.0
            progress_state: dict[str, Any] = context.user_data.get(KEY_PROGRESS, {})
            progress_state["stage"] = "Subiendo resultado"
            progress_state["current"] = min(upload_size, upload_size)
            progress_state["total"] = upload_size
            progress_state["details"] = f"{part_index}/{total_parts} partes subidas"
            progress_state["cancellable"] = True
        ...
        sent_ids = await send_path_with_size_control(
            ...
            progress_state=context.user_data.get(KEY_PROGRESS),
        )
        ...
        await upsert_progress(
            ...
            upload_size,
            upload_size,
            f"{max(sent_parts, expected_parts)}/{max(expected_parts, 1)} partes subidas",
            force=True,
            cancellable=True,
    ...
    async def _process_google_drive_link(self, update: Update, context: ContextTypes.DEFAULT_TYPE, drive_url: str) -> None:
        user = update.effective_user
        chat = update.effective_chat
        ...
        if user_lock.locked():
            await tracked_reply(
                update,
                context,
                "Ya existe una operacion activa para este usuario. Espera a que finalice.",
                reply_markup=start_keyboard(),
            )
            return

        workspace = user_workspace(user.id)
        web_inbox = workspace / "web_download_input"
        web_outbox = workspace / "web_download_output"
        shutil.rmtree(web_inbox, ignore_errors=True)
        shutil.rmtree(web_outbox, ignore_errors=True)
        ...
        async with user_lock:
            register_active_task(user.id, asyncio.current_task())
            try:
                await upsert_progress(
                    update,
                    context,
                ...
                # Intentar obtener el tamaño real antes de iniciar la descarga.
                known_total: int = 0
                if not is_google_drive_folder_url(drive_url):
                    _file_id_pre = extract_drive_file_id(drive_url)
                    if _file_id_pre:
                        known_total = await fetch_gdrive_file_size_async(_file_id_pre)
                if known_total > 0:
                    _ps: dict[str, Any] = context.user_data.get(KEY_PROGRESS, {})
                    _ps["total"] = known_total
                    _ps["current"] = 0
                    context.user_data[KEY_PROGRESS] = _ps

                async def _on_download_progress(current_bytes: int) -> None:
                    ps: dict[str, Any] = context.user_data.get(KEY_PROGRESS, {})
                    ps["stage"] = "Descargando desde Google Drive"
                    if known_total > 0:
                        # Tamaño real conocido — porcentaje preciso.
                        ps["total"] = known_total
                        ps["current"] = min(max(0, int(current_bytes)), known_total)
                    else:
                        # Tamaño desconocido — animación pulse.
                        ps["total"] = 1
                        ps["current"] = 0
                    ps["details"] = f"{format_bytes(max(0, int(current_bytes)))} descargados"
                    ps["cancellable"] = True
                    context.user_data[KEY_PROGRESS] = ps

                result = await download_google_drive(
                    drive_url,
                    web_inbox,
                ...
                if not source_path.exists():
                    raise RuntimeError("No se encontro el contenido descargado de Google Drive")

                action_label = "archivo"
                path_to_upload = source_path
                source_size = 0
                if source_path.is_file():
                    source_size = int(source_path.stat().st_size)
                else:
                    source_size = sum(int(path.stat().st_size) for path in source_path.rglob("*") if path.is_file())

                sent_ids: list[int] = []

                if source_path.is_dir():
                    action_label = "carpeta"
                    # Detectar si hay multiples subcarpetas de primer nivel
                    immediate_subdirs = sorted(
                        [d for d in source_path.iterdir() if d.is_dir()],
                        key=lambda d: d.name.lower(),
                    )
                    if len(immediate_subdirs) > 1:
                        # Multiples subcarpetas → comprimir y enviar cada una por separado
                        total_folders = len(immediate_subdirs)
                        for _idx, _subdir in enumerate(immediate_subdirs, start=1):
                            self._raise_if_cancel_requested(context)
                            _zip_base = safe_name(_subdir.name or f"carpeta_{_idx}")
                            _zip_target = web_outbox / f"{_zip_base}.zip"
                            _part_path = await self._zip_path_with_progress(
                                update,
                                context,
                                _subdir,
                                _zip_target,
                                f"Comprimiendo {_idx}/{total_folders}",
                                f"Subcarpeta {_idx}/{total_folders}: {_subdir.name}",
                            )
                            _part_ids = await self._upload_path_for_drive_flow(
                                update,
                                context,
                                _part_path,
                                caption_footer=(
                                    f"<code>{html_escape(_part_path.name)}</code>"
                                    f" ({_idx}/{total_folders})"
                                ),
                            )
                            sent_ids.extend(_part_ids)
                        # Archivos sueltos en la raiz de la carpeta principal → enviar uno por uno
                        _loose_files = sorted(
                            [f for f in source_path.iterdir() if f.is_file()],
                            key=lambda f: f.name.lower(),
                        )
                        for _lf in _loose_files:
                            self._raise_if_cancel_requested(context)
                            _loose_ids = await self._upload_path_for_drive_flow(
                                update,
                                context,
                                _lf,
                                caption_footer=f"<code>{html_escape(_lf.name)}</code>",
                            )
                            sent_ids.extend(_loose_ids)
                    else:
                        # Carpeta simple (un solo directorio o solo archivos sueltos)
                        zip_name_base = safe_name(source_path.stem or source_path.name or "google_drive")
                        zip_target = web_outbox / f"{zip_name_base}.zip"
                        path_to_upload = await self._zip_path_with_progress(
                            update,
                            context,
                            source_path,
                            zip_target,
                            "Comprimiendo descarga",
                            "Compresion de carpeta",
                        )
                        sent_ids = await self._upload_path_for_drive_flow(
                            update,
                            context,
                            path_to_upload,
                            caption_footer=f"<code>{html_escape(path_to_upload.name)}</code>",
                        )
                elif source_size > int(self.cfg.single_upload_max_bytes):
                    action_label = "archivo grande"
                    zip_name_base = safe_name(source_path.stem or source_path.name or "google_drive")
                    zip_target = web_outbox / f"{zip_name_base}.zip"
                    path_to_upload = await self._zip_path_with_progress(
                        update,
                    ...
                    await upsert_progress(
                        ...
                        source_size,
                        max(source_size, 1),
                        f"Archivo listo: {source_path.name}",
                        force=True,
                        cancellable=True,
                    )
                    sent_ids = await self._upload_path_for_drive_flow(
                ...
                await upsert_progress(
                    ...
                    1,
                    1,
                    "Descarga de Google Drive completada y enviada a Telegram.",
                    force=True,
                )

                def _drive_type_label(path: Path, is_dir: bool) -> str:
                    if is_dir:
                        return "Carpeta"
                    ext = path.suffix.lower()
                    if ext in {".mp4", ".mkv", ".avi", ".mov", ".wmv", ".flv", ".webm", ".m4v", ".ts", ".mpg", ".mpeg", ".3gp"}:
                        return "Video"
                    if ext in {".mp3", ".flac", ".ogg", ".wav", ".m4a", ".aac", ".opus", ".wma", ".amr"}:
                        return "Audio"
                    if ext in {".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp", ".tiff", ".svg", ".heic", ".avif"}:
                        return "Imagen"
                    if ext in {".pdf", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx", ".txt", ".rtf", ".odt", ".ods", ".odp", ".csv", ".epub"}:
                        return "Documento"
                    if ext in {".zip", ".rar", ".7z", ".tar", ".gz", ".bz2", ".xz", ".tgz"}:
                        return "Archivo comprimido"
                    if ext in {".exe", ".apk", ".dmg", ".deb", ".rpm", ".msi"}:
                        return "Ejecutable / App"
                    if ext in {".py", ".js", ".ts", ".html", ".css", ".json", ".xml", ".yml", ".yaml", ".sh", ".bat", ".c", ".cpp", ".h", ".java", ".kt", ".go", ".rs"}:
                        return "Codigo fuente"
                    return "Archivo"

                file_type_label = _drive_type_label(source_path, result.is_directory)
                file_display_name = source_path.name
                summary_text = (
                    f"\u2705 <b>Resumen Google Drive</b>\n"
                    f"<b>Nombre:</b> <a href=\"{drive_url}\">{html_escape(file_display_name)}</a>\n"
                    f"<b>Tipo:</b> {file_type_label}\n"
                    f"<b>Tama\u00f1o descargado:</b> {format_bytes(source_size)}\n"
                    f"<b>Mensajes enviados:</b> {len(sent_ids)}"
                )
                await tracked_reply(
                    update,
                    context,
                ...
                if "cancelada" in str(exc).lower():
                    ...
                    await tracked_reply(update, context, "Descarga desde Google Drive cancelada.", reply_markup=start_keyboard())
                else:
                    await upsert_progress(update, context, self.cfg, "Error", 1, 1, f"Fallo: {exc}", force=True)
                    await tracked_reply(
                        update,
                        context,
                        f"No se pudo procesar el enlace de Google Drive. Detalle tecnico: {exc}",
                        reply_markup=start_keyboard(),
                    )
            except asyncio.CancelledError:
                ...
                unregister_active_task(user.id)

    async def _process_mega_link(self, update: Update, context: ContextTypes.DEFAULT_TYPE, mega_url: str) -> None:
        user = update.effective_user
        chat = update.effective_chat
        if not user or not chat:
            return
        ...
        async with user_lock:
            ...
            try:
                ...
                async def _on_mega_progress(current_bytes: int) -> None:
                    ps: dict[str, Any] = context.user_data.get(KEY_PROGRESS, {})
                    ps["stage"] = "Descargando desde Mega"
                    # mega.py descarga en memoria y escribe de golpe; current_bytes
                    # puede ser 0 durante casi toda la descarga. Pasamos el valor
                    # real para que se muestre si el archivo ya esta en disco,
                    # y usamos pulse para la barra animada.
                    if current_bytes >= 1024:
                        ps["current"] = current_bytes
                        ps["total"] = current_bytes  # sin total real; ETA = --
                        ps["details"] = f"{format_bytes(current_bytes)} descargados"
                    else:
                        ps["current"] = 0
                        ps["total"] = 1
                        ps["details"] = "Conectando con Mega..."
                    ps["cancellable"] = True
                    context.user_data[KEY_PROGRESS] = ps

                result = await download_mega(
                    mega_url,
                    web_inbox,
                    on_progress=_on_mega_progress,
                    cancel_requested=lambda: bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)),
                )
                self._raise_if_cancel_requested(context)
                ...
                source_path = result.local_path
                if not source_path.exists():
                    raise RuntimeError("No se encontro el contenido descargado de Mega")
                ...
                if source_path.is_file():
                    ...
                    source_size = sum(int(p.stat().st_size) for p in source_path.rglob("*") if p.is_file())
                ...
                if should_compress:
                    zip_name_base = safe_name(source_path.stem or source_path.name or "mega")
                    zip_target = web_outbox / f"{zip_name_base}.zip"
                    path_to_upload = await self._zip_path_with_progress(
                        update,
                        context,
                ...
                await upsert_progress(
                    ...
                    1,
                    1,
                    "Descarga de Mega completada y enviada a Telegram.",
                    force=True,
                )
                ...
                file_type_label = _type_label(source_path, result.is_directory)
                summary_text = (
                    f"\u2705 <b>Resumen Mega</b>\n"
                    f"<b>Nombre:</b> <a href=\"{mega_url}\">{html_escape(source_path.name)}</a>\n"
                    f"<b>Tipo:</b> {file_type_label}\n"
                    f"<b>Tama\u00f1o descargado:</b> {format_bytes(source_size)}\n"
                    f"<b>Mensajes enviados:</b> {len(sent_ids)}"
                )
                ...
                if "cancelada" in str(exc).lower():
                    await upsert_progress(update, context, self.cfg, "Cancelando", 1, 1, "La operacion fue cancelada por el usuario.", force=True)
                    await tracked_reply(update, context, "Descarga desde Mega cancelada.", reply_markup=start_keyboard())
                else:
                    await upsert_progress(update, context, self.cfg, "Error", 1, 1, f"Fallo: {exc}", force=True)
                    await tracked_reply(
                        update,
                        context,
                        f"No se pudo procesar el enlace de Mega. Detalle tecnico: {exc}",
                        reply_markup=start_keyboard(),
                    )
            except asyncio.CancelledError:
                context.user_data[KEY_CANCEL_REQUESTED] = True
    ...
    async def _process_terabox_link(self, update: Update, context: ContextTypes.DEFAULT_TYPE, terabox_url: str) -> None:
        user = update.effective_user
        chat = update.effective_chat
        if not user or not chat:
            return
        ...
        async with user_lock:
            ...
            try:
                await upsert_progress(
                    update, context, self.cfg,
                    "Descargando desde Terabox", 0, 1,
                    "Resolviendo enlace...",
                    force=True, cancellable=True, pulse=True,
                )
                heartbeat_task = self._start_progress_heartbeat(update, context)

                known_total: int = 0

                async def _on_terabox_progress(current_bytes: int) -> None:
                    nonlocal known_total
                    ps: dict[str, Any] = context.user_data.get(KEY_PROGRESS, {})
                    ps["stage"] = "Descargando desde Terabox"
                    ps["cancellable"] = True
                    if known_total > 0:
                        ps["total"] = known_total
                        ps["current"] = min(max(0, int(current_bytes)), known_total)
                        ps["details"] = (
                            f"{format_bytes(int(current_bytes))} de {format_bytes(known_total)}"
                        )
                    else:
                        ps["total"] = 1
                        ps["current"] = 0
                ...
                result = await download_terabox(
                    terabox_url,
                    web_inbox,
                    on_progress=_on_terabox_progress,
                    cancel_requested=lambda: bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)),
                )
                # Actualizar known_total con el valor real devuelto
                if result.total_bytes > 0:
                    known_total = result.total_bytes
                self._raise_if_cancel_requested(context)

                source_path = result.local_path
                if not source_path.exists():
                    raise RuntimeError("No se encontro el contenido descargado de Terabox")

                action_label = "archivo"
                path_to_upload = source_path
                source_size = int(source_path.stat().st_size) if source_path.is_file() else sum(
                    int(p.stat().st_size) for p in source_path.rglob("*") if p.is_file()
                )
                ...
                if should_compress:
                    zip_name_base = safe_name(source_path.stem or source_path.name or "terabox")
                    zip_target = web_outbox / f"{zip_name_base}.zip"
                    path_to_upload = await self._zip_path_with_progress(
                        update, context, source_path, zip_target,
                        "Comprimiendo descarga", f"Compresion de {action_label}",
                    )
                else:
                    await upsert_progress(
                        update, context, self.cfg, "Preparando subida",
                ...
                sent_ids = await self._upload_path_for_drive_flow(
                    update, context, path_to_upload,
                    caption_footer=f"<code>{html_escape(path_to_upload.name)}</code>",
                )

                await upsert_progress(
                    update, context, self.cfg, "Proceso completado", 1, 1,
                    "Descarga de Terabox completada y enviada a Telegram.", force=True,
                )
                ...
                summary_text = (
                    f"\u2705 <b>Resumen Terabox</b>\n"
                    f"<b>Nombre:</b> <a href=\"{terabox_url}\">{html_escape(source_path.name)}</a>\n"
                    f"<b>Tipo:</b> {_type_label(source_path, result.is_directory)}\n"
                    f"<b>Tama\u00f1o descargado:</b> {format_bytes(source_size)}\n"
                    f"<b>Mensajes enviados:</b> {len(sent_ids)}"
                )
                await tracked_reply(update, context, summary_text, force_new_message=True, is_html=True)

            except RuntimeError as exc:
                if "cancelada" in str(exc).lower():
                    await upsert_progress(update, context, self.cfg, "Cancelando", 1, 1, "La operacion fue cancelada.", force=True)
                    await tracked_reply(update, context, "Descarga desde Terabox cancelada.", reply_markup=start_keyboard())
                else:
                    await upsert_progress(update, context, self.cfg, "Error", 1, 1, f"Fallo: {exc}", force=True)
                    await tracked_reply(
                        update, context,
                        f"No se pudo procesar el enlace de Terabox. Detalle tecnico: {exc}",
                        reply_markup=start_keyboard(),
                    )
            except asyncio.CancelledError:
                context.user_data[KEY_CANCEL_REQUESTED] = True
    ...
    async def queue_file_for_compression(self, update: Update, context: ContextTypes.DEFAULT_TYPE, file_info: dict[str, Any]) -> None:
        ...
        media_extra = f" | {duration_text}" if duration_text else ""
    ...
    async def _ask_compress_name(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Pide al usuario el nombre del archivo comprimido antes de iniciar."""
        files_to_compress: list[dict[str, Any]] = context.user_data.get(KEY_FILES_TO_COMPRESS, [])
        n = len(files_to_compress)
        context.user_data[KEY_MODE] = MODE_WAIT_COMPRESS_NAME
        context.user_data[KEY_COMPRESS_NAME] = None
        context.user_data[KEY_COMPRESS_PASSWORD] = None
        msg = await tracked_reply(
            update, context,
            f"🗜 <b>Nombre del archivo comprimido</b>\n"
            f"Se van a comprimir <b>{n}</b> archivo(s).\n\n"
            "Escribe el nombre que quieres darle al archivo ZIP (sin extensión):",
            reply_markup=waiting_upload_keyboard(),
            is_html=True,
        )
        if msg:
            context.user_data[KEY_COMPRESS_NAME_MSG_ID] = msg.message_id
    ...
    async def finalize_compression(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        async with user_lock:
            ...
            try:
                ...
                archive_name_raw = str(context.user_data.get(KEY_COMPRESS_NAME) or "").strip()
                if not archive_name_raw:
                    archive_name_raw = f"comprimido_{uuid.uuid4().hex[:8]}"
                # Asegurar extensión .zip
                if not archive_name_raw.lower().endswith(".zip"):
                    archive_name_raw = archive_name_raw + ".zip"
                archive_name = safe_name(archive_name_raw) or f"comprimido_{uuid.uuid4().hex[:8]}.zip"
                archive_path = outbox / archive_name

                # Leer contraseña antes de construir el ZIP
                compress_pwd = context.user_data.get(KEY_COMPRESS_PASSWORD)
                ...
                def _build_zip() -> None:
                    ...
                    with zipfile.ZipFile(
                        ...
                        for index, file_path in enumerate(files_for_zip, start=1):
                            ...
                            with file_path.open("rb") as src, zf.open(file_path.name, "w", force_zip64=True) as dst:
                                while True:
                                    if bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                                        raise RuntimeError("Operacion cancelada por el usuario")
                                    chunk = src.read(chunk_size)
                ...
                # --- Encriptar si el usuario pidió contraseña ---
                compress_pwd = context.user_data.get(KEY_COMPRESS_PASSWORD)
                if compress_pwd:
                    # Reemplazar el ZIP plano por un ZIP con cifrado AES-256.
                    # Se mantiene el nombre original elegido por el usuario.
                    encrypted_name = archive_path.name
                    encrypted_path = outbox / (archive_path.stem + ".__enc_tmp__.zip")
                    enc_size = archive_path.stat().st_size
                    await upsert_progress(
                        update, context, self.cfg,
                        "Encriptando archivo",
                        0, max(enc_size, 1),
                        f"Aplicando contraseña AES-256 a {archive_path.name}...",
                        force=True, cancellable=True,
                    )

                    def _repack_with_password() -> None:
                        import pyzipper
                        chunk_size = 8 * 1024 * 1024
                        with zipfile.ZipFile(archive_path, "r") as src_zf, \
                             pyzipper.AESZipFile(encrypted_path, "w",
                                                 compression=pyzipper.ZIP_DEFLATED,
                                                 encryption=pyzipper.WZ_AES,
                                                 allowZip64=True) as dst_zf:
                            dst_zf.setpassword(compress_pwd.encode("utf-8"))
                            for member in src_zf.infolist():
                                if bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                                    raise RuntimeError("Operacion cancelada por el usuario")
                                with src_zf.open(member) as src_file:
                                    with dst_zf.open(member.filename, "w", force_zip64=True) as dst_file:
                                        while True:
                                            if bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                                                raise RuntimeError("Operacion cancelada por el usuario")
                    ...
                    await asyncio.to_thread(_repack_with_password)
                    try:
                        archive_path.unlink(missing_ok=True)
                    except Exception:
                        pass
                    # Renombrar el tmp al nombre original del archivo elegido por el usuario
                    final_encrypted = outbox / encrypted_name
                    try:
                        encrypted_path.rename(final_encrypted)
                    except Exception:
                        final_encrypted = encrypted_path
                    archive_path = final_encrypted
                    archive_name = encrypted_name
                    validate_file_ready(archive_path)

                _prep_size = archive_path.stat().st_size
                await upsert_progress(
                    update,
                    context,
                    self.cfg,
                    ...
                    0,
                    max(_prep_size, 1),
                    f"Tamano final: {format_bytes(_prep_size)} | Preparando partes",
                    force=True,
                    cancellable=True,
                )
                self._raise_if_cancel_requested(context)
                ...
                should_fallback_to_split = (
                    output_size > max(1, int(self.cfg.single_upload_max_bytes))
                )
                if not should_fallback_to_split:
                    ...
                    single_timeout = self._recommended_upload_timeout(output_size, self.cfg.telegram_operation_timeout)

                    async def _send_one() -> Any:
                        return await stream_send_document(
                            context.bot,
                            chat.id,
                            archive_path,
                ...
                if should_fallback_to_split:
                    recommended_part = min(MAX_DIRECT_UPLOAD_BYTES, effective_part_size(self.cfg))
                    all_fallback_sizes = [
                        recommended_part,
                        1024 * 1024 * 1024,
                        512 * 1024 * 1024,
                    ...
                    # Solo intentar tamaños que realmente dividan el archivo
                    fallback_sizes = [s for s in all_fallback_sizes if s < output_size] or [all_fallback_sizes[-1]]
                    fallback_index = 0
                    while True:
                        _cleanup_parts_dir()
                        # Verificar espacio en disco antes de crear copias
                        try:
                            disk_stat = shutil.disk_usage(archive_path.parent)
                            if disk_stat.free < (output_size + 256 * 1024 * 1024):
                                raise RuntimeError(
                                    f"Espacio insuficiente en disco para dividir: "
                                    f"necesario {output_size // (1024*1024)}MB, "
        ...
        context.user_data[KEY_COMPRESS_NAME] = None
        context.user_data[KEY_COMPRESS_PASSWORD] = None
        context.user_data[KEY_COMPRESS_NAME_MSG_ID] = None
        context.user_data[KEY_COMPRESS_PWD_PROMPT_MSG_ID] = None
    ...
    async def process_extract_queue(self, update: Update, context: ContextTypes.DEFAULT_TYPE, password: str | None = None) -> bool:
        ...
        output_root = workspace / "extract_output"
        # Solo limpiar el inbox para evitar acumular archives de sesiones anteriores.
        # output_root NO se limpia aquí: en el flujo de reintento con contraseña,
        # process_extract_queue se llama desde _handle_password_input_locked y necesita
        # conservar los archivos ya extraídos en output_root.
        shutil.rmtree(inbox, ignore_errors=True)
        inbox.mkdir(parents=True, exist_ok=True)
        output_root.mkdir(parents=True, exist_ok=True)
            ...
            async def _upload_worker() -> None:
                while True:
                    ...
                    try:
                        ...
                        # Propagar cancelación de inmediato
                        if bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                            raise
                        ...
                        if can_retry_safely and attempt < max_file_attempts and not bool(context.user_data.get(KEY_CANCEL_REQUESTED, False)):
                            ...
                            async with progress_lock:
                                ...
                                nonlocal total_upload_bytes, total_upload_parts, total_files_to_send, sent_parts
    ...
    async def finalize_extraction(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        async with user_lock:
            register_active_task(user.id, asyncio.current_task())
            # Limpiar estado residual de sesiones anteriores (crashes, reinicios).
            context.user_data[KEY_EXTRACTED_FILES] = []
            context.user_data[KEY_EXTRACT_ARCHIVES_DONE] = 0
            context.user_data["extract_bytes_done"] = 0
            # Limpiar output_root aquí (no en process_extract_queue) para no borrar
            # archivos ya extraídos cuando se reintenta con contraseña.
            user_ws = user_workspace(user.id)
            shutil.rmtree(user_ws / "extract_output", ignore_errors=True)
            try:
                completed = await self.process_extract_queue(update, context)
                if completed:
                    await self._cleanup_progress_message(update, context, force=True, delete_message=True)
            except (RuntimeError, asyncio.CancelledError) as exc:
                is_cancelled = isinstance(exc, asyncio.CancelledError) or "cancelada" in str(exc).lower()
                if is_cancelled:
                    context.user_data[KEY_MODE] = MODE_IDLE
    ...
    async def _handle_password_input_locked(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Limpiar el out_dir del intento fallido para que no queden archivos parciales
        # que confundan a 7z/pyzipper al reintentar con la nueva contraseña.
        # Si el archivo a extraer está DENTRO del output_dir (caso joined_tmp),
        # lo preservamos moviéndolo fuera antes de limpiar.
        archive_was_inside = False
        tmp_archive_backup: Path | None = None
        if output_dir.exists():
            try:
                archive_path.resolve().relative_to(output_dir.resolve())
                archive_was_inside = True
            except ValueError:
                pass
            if archive_was_inside and archive_path.exists():
                tmp_archive_backup = output_dir.parent / f"_pwd_retry_{archive_path.name}"
                try:
                    shutil.move(str(archive_path), str(tmp_archive_backup))
                except Exception:
                    tmp_archive_backup = None
            try:
                shutil.rmtree(output_dir)
            except Exception:
                pass
        output_dir.mkdir(parents=True, exist_ok=True)
        if tmp_archive_backup and tmp_archive_backup.exists():
            restored = output_dir / archive_path.name
            try:
                shutil.move(str(tmp_archive_backup), str(restored))
                archive_path = restored
            except Exception:
                archive_path = tmp_archive_backup
    ...
    async def _send_result_with_actions(self, update: Update, context: ContextTypes.DEFAULT_TYPE, file_path: Path, caption: str) -> None:
        ...
        if file_path.stat().st_size > max(1, int(self.cfg.single_upload_max_bytes)):
            ...
            part_size_override = min(effective_part_size(self.cfg), MAX_DIRECT_UPLOAD_BYTES)
            # Verificar espacio en disco antes de crear copias de partes
            try:
                _disk = shutil.disk_usage(file_path.parent)
                if _disk.free < (file_path.stat().st_size + 256 * 1024 * 1024):
                    raise RuntimeError(
                        f"Espacio insuficiente en disco para dividir: "
                        f"necesario {file_path.stat().st_size // (1024*1024)}MB, "
                        f"disponible {_disk.free // (1024*1024)}MB"
                    )
            except RuntimeError:
                raise
            except Exception:
            ...
            # Limpiar las partes divididas del disco inmediatamente tras subida exitosa
        ...
        async def _send_one() -> Any:
            ...
            single_timeout = self._recommended_upload_timeout(file_path.stat().st_size, self.cfg.telegram_operation_timeout)
            sent_local = await stream_send_document(
                context.bot,
                chat.id,
                file_path,
        ...
        upload_timeout = self._recommended_upload_timeout(file_path.stat().st_size, self.cfg.telegram_operation_timeout)
    ...
    async def _handle_compress_name_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Recibe el nombre del archivo comprimido y pide si quiere contraseña."""
        ...
        if not raw_name:
            await tracked_reply(
                update, context,
                "El nombre no puede estar vacío. Escribe el nombre del archivo ZIP:",
                reply_markup=waiting_upload_keyboard(),
            )
            return
        # Guardar nombre (sin extensión; la extensión se agrega en finalize_compression)
        clean = raw_name.removesuffix(".zip").removesuffix(".ZIP").strip()
        context.user_data[KEY_COMPRESS_NAME] = clean or raw_name
        # Borrar el mensaje de solicitud de nombre si existe
        name_msg_id = context.user_data.get(KEY_COMPRESS_NAME_MSG_ID)
        if chat and name_msg_id:
            try:
                await context.bot.delete_message(chat_id=chat.id, message_id=int(name_msg_id))
            except Exception:
                pass
            context.user_data[KEY_COMPRESS_NAME_MSG_ID] = None
        ...
        await tracked_reply(
            update, context,
            f"✅ Nombre: <b>{html_escape(clean or raw_name)}.zip</b>\n\n"
            "¿Deseas proteger el archivo con contraseña?",
            reply_markup=compress_password_keyboard(),
            is_html=True,
        )

    async def _handle_compress_password_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Recibe la contraseña de compresión y arranca la compresión."""
        if not update.message or not update.message.text:
            return
        chat = update.effective_chat
        if chat:
            try:
                ...
                pass
            # Borrar también el mensaje del bot que pedía la contraseña
            pwd_prompt_id = context.user_data.get(KEY_COMPRESS_PWD_PROMPT_MSG_ID)
            if pwd_prompt_id:
                try:
                    await context.bot.delete_message(chat_id=chat.id, message_id=int(pwd_prompt_id))
                except Exception:
                    pass
                context.user_data[KEY_COMPRESS_PWD_PROMPT_MSG_ID] = None
        ...
        if not password:
            await tracked_reply(
                update, context,
                "La contraseña no puede estar vacía. Escríbela de nuevo:",
                reply_markup=waiting_upload_keyboard(),
            )
            return
        context.user_data[KEY_COMPRESS_PASSWORD] = password
        context.user_data[KEY_MODE] = MODE_WAIT_COMPRESS_FILES
    ...
    async def text_router(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if mode == MODE_WAIT_COMPRESS_NAME:
            await self._handle_compress_name_input(update, context)
            return

        if mode == MODE_WAIT_COMPRESS_PASSWORD:
            await self._handle_compress_password_input(update, context)
            return

        drive_url = extract_google_drive_url(text)
        if drive_url:
            await self._process_google_drive_link(update, context, drive_url)
            return

        mega_url = extract_mega_url(text)
        if mega_url:
            await self._process_mega_link(update, context, mega_url)
            return

        terabox_url = extract_terabox_url(text)
        if terabox_url:
            await self._process_terabox_link(update, context, terabox_url)
            return
    ...
    async def callback_router(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
            ...
            if query.message and update.effective_chat:
                ...
                if ok:
                    try:
                        ...
                        pass
                    # Eliminar el botón de favoritos del mensaje original
                    try:
                        ...
                        logger.warning("No se pudo quitar botón favoritos: %s", exc)
            return
        ...
        if data == CB_FINALIZAR_COMPRESION and mode == MODE_WAIT_COMPRESS_FILES:
            files_q = context.user_data.get(KEY_FILES_TO_COMPRESS, [])
            if not files_q:
                await tracked_reply(update, context, "No hay archivos en cola para comprimir.", reply_markup=compress_keyboard())
                return
            await self._ask_compress_name(update, context)
            return

        if data == CB_COMPRESS_NO_PASSWORD and mode == MODE_WAIT_COMPRESS_NAME:
            # El bot ya tiene el nombre; el usuario no quiere contraseña → comprimir
            context.user_data[KEY_COMPRESS_PASSWORD] = None
            context.user_data[KEY_MODE] = MODE_WAIT_COMPRESS_FILES
            await self.finalize_compression(update, context)
            return

        if data == CB_COMPRESS_YES_PASSWORD and mode == MODE_WAIT_COMPRESS_NAME:
            context.user_data[KEY_MODE] = MODE_WAIT_COMPRESS_PASSWORD
            msg = await tracked_reply(
                update, context,
                "🔒 <b>Contraseña del archivo</b>\nEscribe la contraseña que quieres usar:",
                reply_markup=waiting_upload_keyboard(),
                is_html=True,
            )
            if msg:
                context.user_data[KEY_COMPRESS_PWD_PROMPT_MSG_ID] = msg.message_id
            return
    ...
    async def file_router(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        file_is_archive = is_archive_file(file_info["file_name"])

        if mode == MODE_WAIT_COMPRESS_FILES:
            if file_is_archive:
                # ── Archivos comprimidos NUNCA se re-comprimen ──
                # Auto-switch a extracción: limpiar cola de compresión.
                old_queue = context.user_data.get(KEY_FILES_TO_COMPRESS, [])
                old_msg_id = context.user_data.get(KEY_COMPRESS_QUEUE_MESSAGE_ID)
                context.user_data[KEY_FILES_TO_COMPRESS] = []
                context.user_data[KEY_COMPRESS_QUEUE_MESSAGE_ID] = None
                context.user_data[KEY_LAST_COMPRESS_QUEUE_TEXT] = ""
                context.user_data[KEY_MODE] = MODE_WAIT_EXTRACT_ARCHIVES

                if chat and old_msg_id:
                    try:
                        await context.bot.delete_message(chat_id=chat.id, message_id=int(old_msg_id))
                    except Exception:
                        pass

                notice = "📂 Archivo comprimido detectado. Modo cambiado a extracción automáticamente."
                if old_queue:
                    notice += f"\nSe descartaron {len(old_queue)} archivo(s) de la cola de compresión."
                await tracked_reply(update, context, notice, reply_markup=extract_keyboard())
        ...
        # MODE_IDLE o cualquier otro estado inesperado
        if file_is_archive:
            # Activar extracción automáticamente para archivos comprimidos
            context.user_data[KEY_MODE] = MODE_WAIT_EXTRACT_ARCHIVES
            await tracked_reply(
                update, context,
                "📂 Archivo comprimido detectado. Modo extracción activado automáticamente.\n"
                "Envía más archivos o pulsa Finalizar extracción.",
                reply_markup=extract_keyboard(),
            )
            await self.queue_file_for_extraction(update, context, file_info)
            return

        # Archivo normal: activar compresión automáticamente
        context.user_data[KEY_MODE] = MODE_WAIT_COMPRESS_FILES
        await tracked_reply(
            update, context,
            "🗜 Archivo detectado. Modo compresión activado automáticamente.\n"
            "Envía más archivos o pulsa Finalizar compresión.",
            reply_markup=compress_keyboard(),
        )
        await self.queue_file_for_compression(update, context, file_info)
...
def build_application(token: str, cfg: BotConfig) -> Application:
    ...
    # Pool separado para polling (getUpdates) para que los uploads de archivos
    # pesados no agoten las conexiones y dejen al bot sordo a comandos nuevos.
