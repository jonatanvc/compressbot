...
from .web_downloads import download_google_drive, extract_google_drive_url
        ...
        async with user_lock:
            ...
            try:
                ...
                async def _on_download_progress(current_bytes: int) -> None:
                    progress_state: dict[str, Any] = context.user_data.get(KEY_PROGRESS, {})
                    total_hint = max(int(progress_state.get("total", 1)), current_bytes, 1)
                    progress_state["stage"] = "Descargando desde Google Drive"
                    progress_state["current"] = max(0, int(current_bytes))
                    progress_state["total"] = total_hint
                    progress_state["details"] = f"{format_bytes(max(0, int(current_bytes)))} descargados"
                    progress_state["cancellable"] = True
                    context.user_data[KEY_PROGRESS] = progress_state
                ...
                sent_ids = await self._upload_path_for_drive_flow(
                    ...
                    context,
                    path_to_upload,
                    caption_footer=f"Descargado desde Google Drive: <code>{path_to_upload.name}</code>",
                )
                ...
                await tracked_reply(
                    ...
                    (
                        ...
                        f"Origen: {drive_url}\n"
                        f"Tipo: {'Carpeta' if source_path.is_dir() else 'Archivo'}\n"
                        f"Tamano descargado: {format_bytes(source_size)}\n"
                        f"Salida enviada: {path_to_upload.name}\n"
                        f"Mensajes enviados: {len(sent_ids)}"
                    ),
