...
def load_config() -> BotConfig:
    ...
    telegram_max_upload_bytes = _safe_int("TELEGRAM_MAX_UPLOAD_BYTES", 4000 * 1024 * 1024)
    upload_part_bytes = _safe_int("UPLOAD_PART_BYTES", (2 * 1024 * 1024 * 1024) - (8 * 1024 * 1024))
    ...
    single_upload_max_bytes = _safe_int("SINGLE_UPLOAD_MAX_BYTES", default_single_limit)
    ...
    return BotConfig(
        ...
        max_download_concurrency=max(1, _safe_int("MAX_DOWNLOAD_CONCURRENCY", default_download_concurrency)),
        max_upload_concurrency=max(1, _safe_int("MAX_UPLOAD_CONCURRENCY", default_upload_concurrency)),
        max_retries=max(1, _safe_int("MAX_RETRIES", 4)),
        retry_base_delay=max(0.2, _safe_float("RETRY_BASE_DELAY", 0.8)),
        enable_checksum=_parse_bool(os.getenv("ENABLE_CHECKSUM", "1"), True),
        checksum_max_bytes=max(0, _safe_int("CHECKSUM_MAX_BYTES", 1024 * 1024 * 1024)),
        allowed_user_ids=allowed,
        only_private_chat=_parse_bool(os.getenv("ONLY_PRIVATE_CHAT", "1"), True),
        progress_bar_size=16,
        progress_min_update_seconds=1.5,
        zip_compress_level=min(9, max(0, _safe_int("ZIP_COMPRESS_LEVEL", 1))),
        max_concurrent_updates=max(2, _safe_int("MAX_CONCURRENT_UPDATES", default_updates)),
        max_parallel_heavy_jobs=max(1, _safe_int("MAX_PARALLEL_HEAVY_JOBS", default_parallel_heavy_jobs)),
        telegram_connect_timeout=max(30.0, _safe_float("TELEGRAM_CONNECT_TIMEOUT", 60.0)),
        telegram_read_timeout=max(30.0, _safe_float("TELEGRAM_READ_TIMEOUT", 120.0)),
        telegram_write_timeout=max(30.0, _safe_float("TELEGRAM_WRITE_TIMEOUT", 120.0)),
        telegram_pool_timeout=max(30.0, _safe_float("TELEGRAM_POOL_TIMEOUT", 60.0)),
        telegram_operation_timeout=max(600.0, _safe_float("TELEGRAM_OPERATION_TIMEOUT", 3600.0)),
        ...
        startup_max_attempts=max(1, _safe_int("STARTUP_MAX_ATTEMPTS", 8)),
        startup_retry_delay=max(1.0, _safe_float("STARTUP_RETRY_DELAY", 5.0)),
