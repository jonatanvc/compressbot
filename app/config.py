...
def load_config() -> BotConfig:
    ...
    default_download_concurrency = 2 if cpu_count <= 1 else min(4, cpu_count * 2)
    default_upload_concurrency = 2 if cpu_count <= 1 else min(3, cpu_count)
    default_updates = 8 if cpu_count <= 1 else max(12, cpu_count * 4)
    ...
    return BotConfig(
        ...
        max_concurrent_updates=max(4, int(os.getenv("MAX_CONCURRENT_UPDATES", str(default_updates)))),
