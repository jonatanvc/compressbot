...
def load_config() -> BotConfig:
    ...
    # Default seguro: muchos despliegues reales se vuelven inestables cerca de ~2GiB en subida directa.
    # Se puede ampliar explicitamente con SINGLE_UPLOAD_MAX_BYTES si tu infraestructura lo soporta.
    default_single_limit = min(telegram_max_upload_bytes, (2 * 1024 * 1024 * 1024) - (16 * 1024 * 1024))
    ...
    return BotConfig(
        ...
        telegram_connect_timeout=max(300.0, float(os.getenv("TELEGRAM_CONNECT_TIMEOUT", "300"))),
        telegram_read_timeout=max(300.0, float(os.getenv("TELEGRAM_READ_TIMEOUT", "300"))),
        telegram_write_timeout=max(300.0, float(os.getenv("TELEGRAM_WRITE_TIMEOUT", "300"))),
        telegram_pool_timeout=max(300.0, float(os.getenv("TELEGRAM_POOL_TIMEOUT", "300"))),
        telegram_operation_timeout=max(
            300.0,
            float(
                os.getenv(
                    "TELEGRAM_OPERATION_TIMEOUT",
                    str(max(300.0, float(os.getenv("TELEGRAM_READ_TIMEOUT", "300")) + float(os.getenv("TELEGRAM_WRITE_TIMEOUT", "300")) + float(os.getenv("TELEGRAM_CONNECT_TIMEOUT", "300")) + float(os.getenv("TELEGRAM_POOL_TIMEOUT", "300")) + 60.0)),
                )
            ),
        ),
