...
def _looks_like_drive_folder(url: str) -> bool:
    return _extract_drive_folder_id(url) is not None
...
async def download_google_drive(
    ...
    try:
        ...
        if _looks_like_drive_folder(url):
            ...
            all_files = [f for f in folder_out.rglob("*") if f.is_file() and not _is_html_error_page(f)]
            if not all_files:
                raise RuntimeError(
            ...
            return WebDownloadResult(source_url=url, local_path=folder_out, is_directory=True)
