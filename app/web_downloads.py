    ...
    try:
        ...
        if is_google_drive_folder_url(url):
            ...
            try:
                ...
                if "cannot retrieve the public link" in _em or "many accesses" in _em or "have had many" in _em:
                    raise RuntimeError(
...
async def _terabox_resolve_direct_url(share_url: str) -> tuple[str, str]:
    """
    Usa la API publica de savetube para resolver el enlace compartido y
    obtener la URL de descarga directa + nombre de archivo.
    Devuelve (download_url, filename).
    """
    import httpx  # type: ignore[import-not-found]
    ...
    # Estructura: {"response": [{"title": "...", "resolutions": {"Fast Download": "url", ...}}]}
    try:
        ...
        if not dl_url:
            raise ValueError("Sin URL de descarga en la respuesta de la API")
        return dl_url, filename
    except (KeyError, IndexError, StopIteration, TypeError) as exc:
        raise RuntimeError(
            "No se pudo obtener el enlace de descarga de Terabox. "
            f"Respuesta inesperada de la API: {str(data)[:300]}"
        ) from exc
