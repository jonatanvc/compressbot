...
def ensure_defaults(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_data.setdefault(KEY_COMPRESS_NAME, None)
    user_data.setdefault(KEY_COMPRESS_PASSWORD, None)
    user_data.setdefault(KEY_COMPRESS_NAME_MSG_ID, None)
...
def is_archive_file(file_name: str) -> bool:
    ...
    if re.search(r"\.(zip|rar)\.\d{3}$", lower_name):
        return True
    # Any .partNNN split file (bot's own format or similar tools)
    if re.search(r"\.part\d{3}$", lower_name):
        return True
