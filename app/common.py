...
def expected_parts_for_size(size_bytes: int, cfg: BotConfig) -> int:
    if size_bytes <= cfg.telegram_max_upload_bytes:
        return 1
    part_size = effective_part_size(cfg)
    return max(1, math.ceil(size_bytes / part_size))


def split_file_for_upload(file_path: Path, cfg: BotConfig) -> list[Path]:
    ...
    original_hash = compute_sha256(file_path) if cfg.enable_checksum else ""
